...
use umls::crypto::test::RustCryptoX25519;

fn main() {
    let mut rng = rand::thread_rng();

    // Create the first user
    let (sig_priv, sig_key) = RustCryptoX25519::generate_sig(&mut rng).unwrap();
    let credential = Credential::Basic(BasicCredential(Opaque::try_from(b"creator").unwrap()));
    let ((kp_priv, kp), make_key_package_0_stack) = stack_usage(|| {
    ...
    let ((kp_priv, kp), make_key_package_1_stack) = stack_usage(|| {
        umls::make_key_package::<RustCryptoX25519>(&mut rng, sig_priv, sig_key, credential).unwrap()
    });
